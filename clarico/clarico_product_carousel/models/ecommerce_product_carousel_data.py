...
class ecommerce_product_carousel_data(osv.osv):
    ...
    def get_product_carousel_slider_filter_data(self,cr,uid,filter_id,object_name,context=None):
        res={}
        if context is None:
            context={}
            res={}
        if not filter_id:
            if object_name == 'product.template':
    ...
    def get_product_for_carousel_slider(self,cr,uid,filter_id,object_name,limit,context=None):
        filter_data = self.get_product_carousel_slider_filter_data(cr, uid, filter_id,object_name, context)
        if filter_data:
