...
class InsTrialBalance(models.TransientModel):
    ...
    show_hierarchy = fields.Boolean(
        string='Show hierarchy', default=False
    )
...
#                            str(q['code'])[:3] == l['code']:
    ...
    def prepare_hierarchy(self, move_lines):
        ...
        def prepare_tmp(id=False, code=False, x_code=False, indent_list=[], parent=[]):
            return {
                'id': id,
                'code': code,
                'x_code': x_code,
        ...
        if move_lines:
            ...
            for line in move_lines:
                ...
                tmp.update(prepare_tmp(id=str(tmp['id']) + 'z1',
                                  code=str(tmp['code'])[0],
                                  x_code=str(tmp['x_code'])[0],
                                  indent_list=[1],
                                  parent=[]))
                if tmp['x_code'] not in [k['x_code'] for k in hirarchy_list]:
                    hirarchy_list.append(tmp)
                    parent_1 = [tmp['id']]
                ...
                final_parent = ['0'] + parent_1 + parent_2 + parent_3
                tmp = q.copy()
                tmp.update({'code': str(tmp['code']), 'x_code': str(tmp['x_code']), 'parent': ' a'.join(final_parent), 'dummy': False, 'indent_list': [1, 2, 3, 4],})
                hirarchy_list.append(tmp)

            for line in move_lines:
                q = move_lines[line]
                for l in hirarchy_list:
                    if str(q['x_code'])[0] == l['x_code'] or \
                            str(q['x_code'])[:3] == l['x_code'] and \
                        ...
                        l['ending_credit'] = q['ending_credit']
                        l['ending_balance'] = q['ending_balance']
            result = sorted(hirarchy_list, key=itemgetter('x_code'))
            new_result = []
            for line in result:
...
#                print"line: ",line
    ...
    def process_data(self, data):
        if data:
            ...
            move_lines = {x.code: {'name':x.name,'code':x.code,'id':x.id,'x_code':x.x_code,
...
#                print"WHERE_CURRENT: ",WHERE_CURRENT
                ...
                move_lines[account.code]['x_code'] = account.x_code
...
#                        print"remvoing: "
                ...
                move_lines[code]['x_code'] = group_account.x_code
