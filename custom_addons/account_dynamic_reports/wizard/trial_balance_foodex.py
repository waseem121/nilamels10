...
#                needed_codes = move_lines.keys()
    ...
    #            for head in heads:
    #                account_level1 = Account.search([('parent_id','=',head.id)],order='code')
    #                for level1 in account_level1:
    #                    if level1.type=='view':
    #                        level2= Account.search([('parent_id','=',level1.id)],order='code')
    #                        for a_level2 in level2:
    #                            if a_level2.type=='view':
    #                                level3= Account.search([('parent_id','=',a_level2.id)],order='code')
    #                                for a_level3 in level3:
    #                                    if a_level3.type=='view':
    #                                        level4= Account.search([('parent_id','=',a_level3.id)],order='code')
    #                                        if level4.type=='view':
    #                                            level5= Account.search([('parent_id','=',level4.id)],order='code')
    #                                            if level5.type=='view':
    #                                                level6= Account.search([('parent_id','=',level5.id)],order='code')
    #                            else:
    #                                sorted_accounts.append(a_level2.code)
    #                    else:
    #                        level2= Account.search([('parent_id','=',level1.id)],order='code')
    #                        
    #                        sorted_accounts.append(level1.code)
    #            sorted_accounts = []
    #            for head in heads:
    ##                print"head name: ",head.name
    ##                child_ids = head._get_children_and_consol_only_leaf_account()
    #                child_ids = head._get_children_and_consol()
    ##                child_ids = Account.search([('parent_id','=',head.id)])
    ##                print"child_ids: ",len(child_ids)
    ...
    #            print"all_accounts len: ",len(all_accounts)
    #            print"all_accounts zero: ",all_accounts[0].code

    #            all_types = self.env['account.account.type'].search([])
    #            all_type_names = []
    #            for type in all_types:
    #                all_type_names.append(type.name)
    #            
