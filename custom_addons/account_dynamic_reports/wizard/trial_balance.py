...
class InsTrialBalance(models.TransientModel):
    ...
    show_hierarchy = fields.Boolean(
        string='Show hierarchy', default=False  #TODO True
    )
...
#                print"line: ",line
    ...
    def process_data(self, data):
        if data:
            ...
            move_lines = {x.code: {'name':x.name,'code':x.code,'serial_no':x.serial_no,'id':x.id,
            ...
            remove_lines = []
            added_codes = []
            print"data.get('display_accounts'): ",data.get('display_accounts')
...
#                if (account.user_type_id.include_initial_balance and self.strict_range) or \
#                        account.user_type_id.name in ('Income View','Income','Other Income', 'Expenses','Expense View','Cost of Revenue','Depreciation'):
                if account.user_type_id.name in ('Income View','Income','Other Income', 'Expenses','Expense View','Cost of Revenue','Depreciation'):
#                if account.user_type_id.name in ('Income View','Income','Other Income', 'Expenses','Expense View'):
                            
                    u_debit +=  init_blns['initial_debit']
                    u_credit +=  init_blns['initial_credit']
#                    u_balance +=  init_blns['initial_balance']
                    
                    
                    init_blns['initial_balance'] = 0
                    init_blns['initial_debit'] = 0
                    init_blns['initial_credit'] = 0
                ...
                if account.type =='view':
                    move_lines[account.code]['type'] = 'view'
                else:
                    move_lines[account.code]['type'] = 'normal'
                    move_lines[account.code]['len'] = 'normal'
                    
#                if account.name=='Income':
#                    print"account.user_type_id.include_initial_balanceeeeeeeeeeeeeeeee: ",account.user_type_id.include_initial_balance
#                    print"self.strict_rangeeeeeeeeeeeeeeeeee: ",self.strict_range
#                    print"account.user_type_id.nameee: ",account.user_type_id.name
                ...
                if account.user_type_id.include_initial_balance and self.strict_range:
                    ...
                    #init_blns = 0.0
#                if not (account.user_type_id.include_initial_balance and self.strict_range) or \
#                        account.user_type_id.name in ('Income View','Income','Other Income', 'Expenses','Expense View'):
...
#                if account.name=='Income':
#                    print"account.user_type_id.include_initial_balanceeeeeeeeeeeeeeeee: ",account.user_type_id.include_initial_balance
#                if not (account.user_type_id.name in ('Income View','Income','Other Income', 'Expenses','Expense View')):
#                    if account.name=='Income':
#                        print"insidee iffffffff"
...
#                    end_dr = deb
                ...
                move_lines[account.code]['indent_list'] = self._get_indent_list(account)
...
#                        print"remvoing: "
                ...
                move_lines[code]['type'] = 'view'
                move_lines[code]['code'] = code
                move_lines[code]['serial_no'] = group_account.serial_no
                ...
                move_lines[code]['indent_list'] = self._get_indent_list(group_account)
                ...
                for type in account_types:
                    ...
                    for a in type_account_ids:
                        ...
                        if a.serial_no:
                            ...
                            v['indent_list'] = self._get_indent_list(a)
                            account_list.append(v)
                    account_ids.extend(type_account_ids.ids)
#                print"len account_ids: ",len(account_ids)


            ## sorting based on serial no
            view_accounts = Account.search([('type','=','view'),
                            ('serial_no','!=',False)
                            ], order='serial_no asc')
#                print"view_accounts: ",view_accounts

            for view_account in view_accounts:
                if not view_account.serial_no:
                    continue
                print"view_account name ",view_account.name
                if view_account.code not in sorted_accounts:
                    v={}
                    v['code'] = view_account.code
                    v['serial_no'] = view_account.serial_no
                    v['indent_list'] = self._get_indent_list(view_account)
                    account_list.append(v)

                exist = move_lines.get(view_account.code,False)
                if not exist:
                    template = {'name': 'view account',
                        ...
                        'company_currency_id': company_currency_id.id}
                    view_line=template
                    view_line['name'] = view_account.name
                    view_line['code'] = view_account.code
                    view_line['serial_no'] = view_account.serial_no
                    view_line['id'] = view_account.id
                    view_line['indent_list'] = self._get_indent_list(view_account)
                    move_lines[view_account.code] = view_line

            account_list = sorted(account_list, key = lambda i: i['serial_no'])
#                print"account_list: ",account_list
            new_sorted = []
            for d in account_list:
                if d.get('code') not in new_sorted:
                    new_sorted.append(d.get('code'))
            sorted_accounts = new_sorted
            sorted_accounts = list(set(sorted_accounts))


        ## for sorting
#            indent_list=[1, 2, 3]
            new_list = []
            for code in sorted_accounts:
                account = Account.search([('code','=',code)])
                v = {}
                v['code'] = account.code
                v['serial_no'] = account.serial_no
                v['indent_list'] = self._get_indent_list(account)
                new_list.append(v)

            new_list = sorted(new_list, key = lambda i: i['serial_no'])
            last_sorted = []
            for d in account_list:
                if d.get('code') not in last_sorted:
                    last_sorted.append(d.get('code'))
            sorted_accounts = last_sorted


            #### add sum of child accounts to its Parent
            normal_accounts = Account.search(['|',('type','!=','view'),
                        ('group_account','=',True)
                    ])
#            print"u debittttttttttTT: ",u_debit
#            print"u credd tttttttttTT: ",u_credit
#            print"u credd balanceeeee: ",u_balance
            for account in normal_accounts:
                line = move_lines.get(account.code,False)
                if line:
                    if account.name=='Undistributed Profit & Loss':
                        print"line : ",line
                        
                        line['initial_debit'] = u_credit
                        line['initial_credit'] = u_debit
                        line['initial_balance'] = (line['initial_debit']-line['initial_credit'])
                        
#                        line['credit'] = (line['initial_debit']-line['initial_credit'])
#                        line['balance'] = (line['debit']-line['credit'])
                        
#                        line['ending_debit'] = line['debit']
#                        line['ending_credit'] = line['credit']
#                        line['ending_balance'] = (line['debit']-line['credit'])
                        
#                        line['debit'] = u_credit
#                        line['credit'] = u_debit
#                        line['balance'] = (line['debit']-line['credit'])
                        print"line After: ",line
                        move_lines[account.code] = line
                        
#                        print"line: ",line
                    parent = account.parent_id or False
                    while parent:
                        line1 = move_lines.get(parent.code,False)
                        if line1:
                            line1 = self._update_line(line, line1)
                            move_lines[parent.code]=line1
...
#            print"move_lines: ",move_lines
    ...
    def _update_line(self, line, line1):
        debit = line.get('debit')
        credit = line.get('credit')
        balance = line.get('balance')
        
        initial_debit = line.get('initial_debit')
        initial_credit = line.get('initial_credit')
        initial_balance = line.get('initial_balance')
        
        ending_debit = line.get('ending_debit')
        ending_credit = line.get('ending_credit')
        ...
        line1['initial_debit'] += initial_debit
        line1['initial_credit'] += initial_credit
        line1['initial_balance'] += initial_balance
        
        
        line1['ending_debit'] += ending_debit
        line1['ending_credit'] += ending_credit
        line1['ending_balance'] += ending_balance
