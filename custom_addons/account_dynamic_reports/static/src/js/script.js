...
var _t = core._t;
var exports = {};
var DynamicFrMain = Widget.extend({
...
'click #date_from': 'preventDef',
'click #date_to': 'preventDef',
'click #date_from_cmp': 'preventDef',
'click #date_to_cmp': 'preventDef'
},
...
if (!self.wizard_id) {
new Model('ins.financial.report').call('create', [{
...
e.preventDefault();
var self = this;
new Model('ins.financial.report').call('get_report_values', [[self.wizard_id]])
.then(function(data) {
var action = {
...
print_xlsx: function() {
var self = this;
new Model('ins.financial.report').call('action_xlsx', [[self.wizard_id]])
...
var currency_id = session.get_currency(formatOptions.currency_id);
var without_sign = formats.format_value(Math.abs(amount), {type: "float", digits: currency_id.digits});
if (!amount) {
return '-'
};
...
var fr_wizard_id = 0;
new Model('ins.general.ledger').call('create', [{}])
.then(function(record) {
fr_wizard_id = record;
new Model('ins.general.ledger').call('write', [fr_wizard_id, domains])
.then(function() {
var action = {
...
new Model('ins.financial.report').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('ins.general.ledger').call('create', [{res_model: this.res_model}])
.then(function(record) {
self.wizard_id = record;
self.plot_data(self.initial_render);
})
...
e.preventDefault();
var self = this;
new Model('ins.general.ledger').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
...
print_xlsx: function() {
var self = this;
new Model('ins.general.ledger').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
k.credit = self.formatWithSign(k.credit, formatOptions, k.credit < 0 ? '-' : '');
k.balance = self.formatWithSign(k.balance, formatOptions, k.balance < 0 ? '-' : '');
k.ldate = formats.format_value(k.ldate, {type:"date"});
...
ks.ldate = formats.format_value(ks.ldate, {type:"date"});
});
});
if (initial_render) {
...
var self = this;
return new Model('ins.general.ledger').call('build_detailed_move_lines', [self.wizard_id, offset, account_id])
},
...
new Model('ins.general.ledger').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('ins.partner.ageing').call('create', [{res_model: this.res_model}])
.then(function(record) {
self.wizard_id = record;
self.plot_data(self.initial_render);
})
...
e.preventDefault();
var self = this;
new Model('ins.partner.ageing').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
...
print_xlsx: function() {
var self = this;
new Model('ins.partner.ageing').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
sorted_partner:self.sorted_partner
}));
self.loader_enable_ui();
});
},
...
var self = this;
return new Model('ins.partner.ageing').call('process_detailed_data', [self.wizard_id, offset, account_id])
},
fetch_move_lines_by_page: function(event) {
event.preventDefault();
var self = this;
...
k.date_maturity = formats.format_value(k.date_maturity, {type:"date"});
});
$(event.currentTarget).parent().parent().parent().find('.py-mline-table-div').remove();
$(event.currentTarget).parent().parent().find('a').css({
'background-color': 'white',
...
$(event.currentTarget).next('tr').find('td .py-mline-table-div').remove();
$(event.currentTarget).next('tr').find('td ul').after(QWeb.render('SubSectionPa', {
count: count,
offset: offset,
account_data: account_data,
period_list: period_list
...
new Model('ins.partner.ageing').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('ins.partner.ledger').call('create', [{
res_model: this.res_model
}])
.then(function(record) {
self.wizard_id = record;
...
e.preventDefault();
var self = this;
new Model('ins.partner.ledger').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
...
print_xlsx: function() {
var self = this;
new Model('ins.partner.ledger').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
var self = this;
return new Model('ins.partner.ledger').call('build_detailed_move_lines', [self.wizard_id, offset, account_id])
},
fetch_move_lines_by_page: function(event) {
event.preventDefault();
var self = this;
...
new Model('ins.partner.ledger').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
//
//Trial balance
var DynamicTbMain = Widget.extend({
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('ins.trial.balance').call('create', [{res_model: this.res_model}])
.then(function(record) {
self.wizard_id = record;
self.plot_data(self.initial_render);
})
...
e.preventDefault();
var self = this;
new Model('ins.trial.balance').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
...
print_xlsx: function() {
var self = this;
new Model('ins.trial.balance').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
self.filter_data.date_from = formats.format_value(self.filter_data.date_from, {type:"date"});
self.filter_data.date_to = formats.format_value(self.filter_data.date_to, {type:"date"});
if (initial_render) {
...
.then(function(record) {
gl_wizard_id = record;
new Model('ins.general.ledger').call('write', [gl_wizard_id, domains])
.then(function() {
var action = {
type: 'ir.actions.client',
name: 'GL View',
...
new Model('ins.trial.balance').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
//
//
var DynamicPprMain = Widget.extend({
template: 'DynamicPprMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('product.profit.report').call('create', [{
res_model: this.res_model
}])
.then(function(record) {
self.wizard_id = record;
...
e.preventDefault();
var self = this;
new Model('product.profit.report').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
'report_name': 'account_dynamic_reports.product_profit',
'report_file': 'account_dynamic_reports.product_profit',
'data': {
'js_data': data
},
'context': {
'active_model': 'product.profit.report',
'landscape': 1,
'from_js': true
},
'display_name': 'Product Profit Report',
};
return self.do_action(action);
});
},
print_xlsx: function() {
var self = this;
new Model('product.profit.report').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
.then(function(datas) {
self.filter_data = datas[0]
self.categ_data = datas[1]
_.each(self.categ_data, function(k, v) {
var formatOptions = {
currency_id: k.company_currency_id,
noSymbol: true,
};
...
if (initial_render) {
self.$('.py-control-panel').html(QWeb.render('FilterSectionPpr', {
filter_data: datas[0]
}));
self.$el.find('#date_from').datepicker({
dateFormat: 'dd-mm-yy'
...
self.$('.py-data-container-orig').html(QWeb.render('DataSectionPpr', {
categ_data: datas[1]
}));
self.loader_enable_ui();
});
},
pl_lines_by_page: function(offset, categ_id) {
var self = this;
return new Model('product.profit.report').call('build_detailed_move_lines', [self.wizard_id, offset, categ_id])
},
fetch_move_lines_by_page: function(event) {
event.preventDefault();
var self = this;
var categ_id = $(event.currentTarget).data('categ-id');
var offset = parseInt($(event.currentTarget).data('page-number')) - 1;
var total_rows = parseInt($(event.currentTarget).data('count'));
self.loader_disable_ui();
self.pl_lines_by_page(offset, categ_id).then(function(datas) {
_.each(datas[2], function(k, v) {
var formatOptions = {
currency_id: k.company_currency_id,
noSymbol: true,
};
k.qty = self.formatWithSign(k.qty, formatOptions, k.qty < 0 ? '-' : '');
});
$(event.currentTarget).parent().parent().parent().find('.py-mline-table-div').remove();
$(event.currentTarget).parent().parent().find('a').css({
'background-color': 'white',
...
redirect_to_document('product.product', $(event.currentTarget).data('product-id'));
},
update_with_filter: function(event) {
event.preventDefault();
var self = this;
...
//mycode
var product_ids = [];
var product_list = $(".partner-multiple").select2('data')
...
new Model('product.profit.report').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
//
//
// Bills profit report start
//
//
var DynamicBprMain = Widget.extend({
template: 'DynamicBprMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('bills.profit.report').call('create', [{
res_model: this.res_model
}])
.then(function(record) {
self.wizard_id = record;
...
e.preventDefault();
var self = this;
new Model('bills.profit.report').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
'report_name': 'account_dynamic_reports.bills_profit',
'report_file': 'account_dynamic_reports.bills_profit',
'data': {
'js_data': data
},
'context': {
'active_model': 'bills.profit.report',
'landscape': 1,
'from_js': true
},
'display_name': 'Bills Profit Report',
};
return self.do_action(action);
});
},
print_xlsx: function() {
var self = this;
new Model('bills.profit.report').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
if (initial_render) {
self.$('.py-control-panel').html(QWeb.render('FilterSectionBpr', {
filter_data: datas[0]
}));
self.$el.find('#date_from').datepicker({
dateFormat: 'dd-mm-yy'
...
self.$('.py-data-container-orig').html(QWeb.render('DataSectionBpr', {
categ_data: datas[1]
}));
self.loader_enable_ui();
});
...
pl_lines_by_page: function(offset, categ_id) {
var self = this;
return new Model('bills.profit.report').call('build_detailed_move_lines', [self.wizard_id, offset, categ_id])
},
fetch_move_lines_by_page: function(event) {
event.preventDefault();
var self = this;
...
redirect_to_document('account.invoice', $(event.currentTarget).data('product-id'));
},
update_with_filter: function(event) {
event.preventDefault();
var self = this;
...
new Model('bills.profit.report').call('write', [self.wizard_id, output])
.then(function(res) {
self.plot_data(self.initial_render);
});
},
...
// Bills profit report end
//
//
// Bills profit report Group start
// Bills profit report Group start
var DynamicBprsMain = Widget.extend({
template: 'DynamicBprsMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
if (initial_render) {
self.$('.py-control-panel').html(QWeb.render('FilterSectionBprs', {
filter_data: datas[0]
}));
self.$el.find('#date_from').datepicker({
dateFormat: 'dd-mm-yy'
...
self.$('.py-data-container-orig').html(QWeb.render('DataSectionBprs', {
categ_data: datas[1]
}));
self.loader_enable_ui();
});
...
/// bills profit report group end
/// bills profit report group end


// product profit Non group start
var DynamicPprwgMain = Widget.extend({
template: 'DynamicPprwgMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
e.preventDefault();
var self = this;
new Model('product.profit.report').call('get_report_datas_wg', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
...
if (initial_render) {
self.$('.py-control-panel').html(QWeb.render('FilterSectionPprwg', {
filter_data: datas[0]
}));
self.$el.find('#date_from').datepicker({
dateFormat: 'dd-mm-yy'
...
self.$('.py-data-container-orig').html(QWeb.render('DataSectionPprwg', {
categ_data: datas[1]
}));
self.loader_enable_ui();
});
...
// product profit Non group end

// product Activity Report start
var DynamicParMain = Widget.extend({
template: 'DynamicParMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('product.activity.report').call('create', [{
res_model: this.res_model
}])
.then(function(record) {
self.wizard_id = record;
...
e.preventDefault();
var self = this;
new Model('product.activity.report').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
'report_name': 'account_dynamic_reports.product_activity',
'report_file': 'account_dynamic_reports.product_activity',
'data': {
'js_data': data
},
'context': {
'active_model': 'product.activity.report',
'landscape': 1,
'from_js': true
},
'display_name': 'Product Activity Report',
};
return self.do_action(action);
});
},
print_xlsx: function() {
var self = this;
new Model('product.activity.report').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
if (initial_render) {
self.$('.py-control-panel').html(QWeb.render('FilterSectionPar', {
filter_data: datas[0]
}));
self.$el.find('#date_from').datepicker({
dateFormat: 'dd-mm-yy'
...
self.$('.py-data-container-orig').html(QWeb.render('DataSectionPar', {
categ_data: datas[1]
}));
self.loader_enable_ui();
});
...
// product Activity report end

// product stock report start
var DynamicPsrMain = Widget.extend({
template: 'DynamicPsrMain',
events: {
'click #filter_apply_button': 'update_with_filter',
'click #pdf': 'print_pdf',
'click #xlsx': 'print_xlsx',
...
self.initial_render = true;
if (!self.wizard_id) {
new Model('product.stock.report').call('create', [{
res_model: this.res_model
}])
.then(function(record) {
self.wizard_id = record;
...
e.preventDefault();
var self = this;
new Model('product.stock.report').call('get_report_datas', [[self.wizard_id]])
.then(function(data) {
var action = {
'type': 'ir.actions.report.xml',
'report_type': 'qweb-pdf',
'report_name': 'account_dynamic_reports.product_stock',
'report_file': 'account_dynamic_reports.product_stock',
'data': {
'js_data': data
},
'context': {
'active_model': 'product.stock.report',
'landscape': 1,
'from_js': true
},
'display_name': 'Product Stock Report',
};
return self.do_action(action);
});
},
print_xlsx: function() {
var self = this;
new Model('product.stock.report').call('action_xlsx', [[self.wizard_id]])
.then(function(action) {
action.context.active_ids = [self.wizard_id];
return self.do_action(action);
});
...
if (initial_render) {
self.$('.py-control-panel').html(QWeb.render('FilterSectionPsr', {
filter_data: datas[0]
}));
self.$el.find('#date_from').datepicker({
dateFormat: 'dd-mm-yy'
...
self.$('.py-data-container-orig').html(QWeb.render('DataSectionPsr', {
categ_data: datas[1]
}));
self.loader_enable_ui();
});
...
pl_lines_by_page: function(offset, categ_id) {
var self = this;
return new Model('product.stock.report').call('build_detailed_move_lines', [self.wizard_id, offset, categ_id])
},
fetch_move_lines_by_page: function(event) {
event.preventDefault();
var self = this;
...
core.action_registry.add('dynamic.fr', DynamicFrMain);core.action_registry.add('dynamic.gl', DynamicGlMain);core.action_registry.add('dynamic.pa', DynamicPaMain);core.action_registry.add('dynamic.pl', DynamicPlMain);core.action_registry.add('dynamic.tb', DynamicTbMain);core.action_registry.add('dynamic.ppr', DynamicPprMain);core.action_registry.add('dynamic.bpr', DynamicBprMain);core.action_registry.add('dynamic.pprwg', DynamicPprwgMain);core.action_registry.add('dynamic.par', DynamicParMain);core.action_registry.add('dynamic.psr', DynamicPsrMain);core.action_registry.add('dynamic.bprs', DynamicBprsMain);return DynamicFrMain;});
