...
class ProductTemplate(models.Model):
    ...
    def _compute_quantities(self):
        ...
        for template in self:
            ...
            template.virtual_available_inv = res[template.id]['virtual_available_inv']
    ...
    def _compute_quantities_dict(self):
        ...
        for template in self:
            ...
            incoming_qty = 0
            outgoing_qty = 0
            virtual_available_inv = 0
            for p in template.product_variant_ids:
                qty_available += variants_available[p.id]["qty_available"]
                virtual_available += variants_available[p.id]["virtual_available"]
                incoming_qty += variants_available[p.id]["incoming_qty"]
                outgoing_qty += variants_available[p.id]["outgoing_qty"]
                virtual_available_inv += variants_available[p.id]["virtual_available_inv"]
#                
#                location_id = self.env.context.get('location', False)
#                draft_inv_qty = self.env['account.invoice'].get_draft_invoice_qty(product,location_id)
#                virtual_available_inv += (qty_available - outgoing_qty) - draft_inv_qty
            ...
            prod_available[template.id] = {
                ...
                "virtual_available_inv": virtual_available_inv,
            }
...
class product_product(models.Model):
    ...
    virtual_available_tranfer_inv = fields.Float(
        'Forecast Quantity Tr Inv', compute='_compute_quantities',
        digits=dp.get_precision('Product Unit of Measure'),
    ...
    # inherited to add virtual_available_transfer_inv
    ...
    def _compute_quantities(self):
        ...
        for product in self:
            ...
            product.virtual_available_tranfer_inv = res[product.id]['virtual_available_tranfer_inv']
...
#            print"foodex/product/qty_available: ",qty_available
#            print"foodex/product/outgoing: ",res[product.id]['outgoing_qty']                

#            print"foodex/product/Incoming Qty ",res[product.id]['incoming_qty']
            location_id = self.env.context.get('location', False)
            draft_inv_qty = self.get_draft_invoice_qty(product,location_id)
#            print"foodex/product draft_inv_qty: ",draft_inv_qty
            ...
            res[product.id]['virtual_available_tranfer_inv'] = (float_round(qty_available+res[product.id]['incoming_qty'] - res[product.id]['outgoing_qty'],
                precision_rounding=product.uom_id.rounding)) - draft_inv_qty
        return res
    
    #inherited to add loation
    def get_draft_invoice_qty(self,  product, location):
#        settings = self.env['account.config.settings'].search([],order='id DESC')
#        days_draft_invoice_from = settings[0].days_draft_invoice_from or False
        days_draft_invoice_from = 90
        days_draft_invoice_from = (datetime.today() - timedelta(days=days_draft_invoice_from)).date()
...
#            return 0.0
        ...
        if location:
            self._cr.execute("select l.id from account_invoice_line l, account_invoice a where l.invoice_id=a.id "\
                ...
                "a.location_id in %s and a.date_invoice>=%s",(product.id,tuple(location_ids),days_draft_invoice_from))
            res = self._cr.fetchall()
        else:
            self._cr.execute("select l.id from account_invoice_line l, account_invoice a where l.invoice_id=a.id "\
                "and a.state in ('draft','proforma','proforma2') and a.type in ('out_invoice', 'out_refund') and l.product_id=%s and a.date_invoice>=%s"\
                ,(product.id,days_draft_invoice_from))
            res = self._cr.fetchall()
        if not res:
            return 0.0
#        line_ids = self._cr.fetchone()[0] or 0.0        
#        line_ids = res
        line_ids = [r[0] for r in res]
        in_qty, out_qty, total_draft_qty = 0.0,0.0,0.0
        line_obj = self.env['account.invoice.line']
        for line in line_obj.browse(line_ids):
#            print"line.ivnocie_id: ",line.invoice_id
        ...
        _logger.info('IT IS INFO Execution time (%s)',(time.time() - start_time,))
        _logger.debug('IT IS Debug Execution time (%s)',(time.time() - start_time,))
