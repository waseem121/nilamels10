...
class StockPicking(models.Model):
    ...
    def action_open_landed_cost(self):
        ...
        if not lines:
            ...
            cost_dist = cost_dist_obj.create({'exchange_rate':exchange_rate,'picking_id':self.id})
...
class StockMove(models.Model):
    ...
    # to create the Difference adjustment entry from the landed cost calculation
#    @api.multi
...
#        print"purchaselanded action done called"
#        if picking:
...
#                        all_credit_line_ids = AccountMove.line_ids.filtered(lambda l:l.credit)
#                        total_credited = sum(all_credit_line_ids.mapped('credit'))
#                        print"total_credited: ",total_credited
#                        if total_credited != difference_amount:
#                        
#                            price_diff_line = {
...
#    #
#    #                        credit = sum(credit_line_ids.mapped('credit')) + difference_amount
#    #                        credit_line_ids.with_context(
#    #                                {'check_move_validity':False}).write({'credit':credit,
#    #                                                                    'debit':0,
#    #                                                                    })
#
#    #                        debit = sum(debit_line_ids.mapped('debit')) + difference_amount
...
#                            credit_line_ids = AccountMove.line_ids.filtered(lambda l:l.account_id.id not in expense_account_ids and l.credit)
#                            if difference_amount> 0.0:
#                                credit = sum(credit_line_ids.mapped('credit')) - abs(difference_amount)
#                            else:
#                                credit = sum(credit_line_ids.mapped('credit')) + abs(difference_amount)
#                            credit_line_ids.with_context(
#                                    {'check_move_validity':False}).write({'credit':credit,
...
#                        'ref': name})
                if AccountMove:
                    if not len(expense_lines):
                        print"continueee"
                        continue
                    AccountMove.button_cancel()
                        
#                        for l in AccountMove.line_ids:
#                            l.write({'debit':round(l.debit,3),
#                                'credit':round(l.credit,3)})
...
#                    if round(total_credit,3) != round(total_debit,3):
                    if round(total_credit,6) != round(total_debit,6):
                        ...
                        diff_amount = total_debit - total_credit
                        print"diff_amount: ",diff_amount
                        diff_amount = round(diff_amount, 6)
                        print"diff_amountR: ",diff_amount
                        price_diff_account = move.product_id.property_account_creditor_price_difference
                        if not price_diff_account:
                            price_diff_account = move.product_id.categ_id.property_account_creditor_price_difference_categ
                        if not price_diff_account:
...
#                        print"price_diff_line: ",price_diff_line
                        line_id = self.env['account.move.line'].with_context({'check_move_validity':False}).create(price_diff_line)
                        print"line created: ",line_id
#                    stop
                    AccountMove.post()
...
#            for each_picking_line in pur_dist_line_id:
##                print"each_picking_line: ",each_picking_line
#                exchange_rate = each_picking_line.distribution.exchange_rate
...
##                                                                          round=True)
#                        expense_amount = each_expense.currency_id.compute_custom(each_expense.expense_amount,
#                                                                          each_picking_line.distribution.currency_id, exchange_rate)
#                    else:
...
##                    print"expense_amount after round: ",expense_amount
#                    land_cost_vals = {
#                        'name': each_expense.type.name,
#                        'product_id': self.product_id.id,
#                        'quantity': qty,
