...
#                    tmp_style_num = self.line_header
                    tmp_style_num = self.three_digits
                else:
                    tmp_style_str = self.line_header_string_bold
#                    tmp_style_num = self.line_header_bold
                    tmp_style_num = self.three_digits_bold
                if a.get('level',0) < 3 or a.get('group_account',False):
                    tmp_style_str = self.line_header_string_bold
#                    tmp_style_num = self.line_header_bold
                    tmp_style_num = self.three_digits_bold
                
                blank_space = '   ' * len(a.get('indent_list'))
                if a.get('level',0) >3 or a.get('group_account',False):
                    self.sheet.write_string(self.row_pos, 0, blank_space+a.get('code'),
                                            tmp_style_str)
                else:
                    self.sheet.write_string(self.row_pos, 0, blank_space+'',
                                            tmp_style_str)
                    
                self.sheet.write_string(self.row_pos, 1, blank_space+a.get('name'),
                                        tmp_style_str)
