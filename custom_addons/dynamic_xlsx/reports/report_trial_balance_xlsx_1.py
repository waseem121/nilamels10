            ...
            if not filter.get('show_hierarchy'):
                for line in sorted_accounts:
                    ...
                    if acc_lines[line].get('type','')=='view':
                        self.sheet.write_string(self.row_pos, 0,  acc_lines[line].get('code') + ' ' +acc_lines[line].get('name'), self.line_header_bold_left)
                        self.sheet.write_number(self.row_pos, 1, float(acc_lines[line].get('initial_debit')), self.line_header_bold)
                        self.sheet.write_number(self.row_pos, 2, float(acc_lines[line].get('initial_credit')), self.line_header_bold)
                        self.sheet.write_number(self.row_pos, 3, float(acc_lines[line].get('initial_balance')), self.line_header_highlight)
                        self.sheet.write_number(self.row_pos, 4, float(acc_lines[line].get('debit')), self.line_header_bold)
                        self.sheet.write_number(self.row_pos, 5, float(acc_lines[line].get('credit')), self.line_header_bold)
                        self.sheet.write_number(self.row_pos, 6, float(acc_lines[line].get('balance')), self.line_header_highlight)
                        self.sheet.write_number(self.row_pos, 7, float(acc_lines[line].get('ending_debit')), self.line_header_bold)
                        self.sheet.write_number(self.row_pos, 8, float(acc_lines[line].get('ending_credit')), self.line_header_bold)
                        self.sheet.write_number(self.row_pos, 9, float(acc_lines[line].get('ending_balance')), self.line_header_highlight)
                        self.sheet.write_string(self.row_pos, 10,  acc_lines[line].get('code'), self.line_header_bold_left)
                    else:
                        ...
                        self.sheet.write_number(self.row_pos, 9, float(acc_lines[line].get('ending_balance')), self.line_header_highlight)
                        self.sheet.write_string(self.row_pos, 10,  acc_lines[line].get('code'), self.line_header_light_left)
                ...
                for line in acc_lines: # Normal lines
                    ...
                    self.sheet.write_string(self.row_pos, 10, line.get('code'),
                                            self.line_header_light_left)
