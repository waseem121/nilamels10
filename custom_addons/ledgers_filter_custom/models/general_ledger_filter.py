...
#    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account, anyalytic_account=None):
        used_context = {'date_from':self.date_from,
                    'date_to':self.date_to,
...
#                date_from=self.env.context.get('date_from'), date_to=False, initial_bal=True)._query_get()
            init_tables, init_where_clause, init_where_params = MoveLine.with_context(
                date_from=self.date_from, date_to=self.date_to, initial_bal=True)._query_get()
            init_wheres = [""]
            ...
            if foreign_currency:
                ...
                sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, NULL AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.amount_currency),0.0) AS debit, COALESCE(SUM(l.amount_currency),0.0) AS credit, COALESCE(SUM(l.amount_currency),0) - COALESCE(SUM(l.amount_currency), 0) as balance, '' AS lpartner_id,\
                    ...
                    WHERE l.account_id IN %s AND l.amount_currency>0""" + filters)
                ...
                for row in cr.dictfetchall():
                    amount_currency_debit = row.get('debit',0)
                ...
                cr.execute(sql, params)
                for row in cr.dictfetchall():
                    amount_currency_credit = row.get('credit',0)
                    amount_currency_credit = amount_currency_credit * -1
                
#            filters = filters+'AND l.date < "2020-06-01"'
            ...
            for row in cr.dictfetchall():
                ...
                if foreign_currency:
                    row['debit'] = amount_currency_debit
                    row['credit'] = amount_currency_credit
                    row['balance'] = amount_currency_debit - amount_currency_credit
        ...
        for account in accounts:
            ...
            if target_move == 'all':
                ...
                for invoice in invoices:
                    ...
                    new_dict['currency_code'] = invoice.currency_id and invoice.currency_id.symbol or ''
...
#            for line in res.get('move_lines'):
            ...
            print"init_credit: ",init_credit
            print"init_debit: ",init_debit
...
#                    if str(line['lname'])[:15] == 'Initial Balance':
            ...
            for line in new_move_lines:
                ...
                if not str(line['lname'])[:15] in ('Opening Balance','Initial Balance'):
...
#            output.write('"%s","%s","%s","%s","%s","%s","%s","%s","%s"' % (name,'','','','','',debit,credit,balance))
        ...
        self.write({'data':out,'name':'Export_SingleGeneralLedger.csv'})
        return {
...
class ReportGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby,
        ...
        if init_balance:
            ...
            amount_currency_credit = 0
            foreign_currency = self.env.context.get('foreign_currency')
            print"foreign_currency: ",foreign_currency
            if foreign_currency:
                filters = filters+' AND l.amount_currency != 0.0'
            ...
            for row in cr.dictfetchall():
                ...
                for line in move_lines.get(row['account_id']):
                    ...
                    if amount_currency > 0.0:
                        ...
                        print"amount_currency-1: ",amount_currency * -1
                        line['credit'] = (amount_currency * -1)
                        print"lineeeee: ",line
                    
                    balance += line['debit'] - line['credit']
                    print"balanceee: ",balance
                row['balance'] += balance
                print"before pop"
                move_lines[row.pop('account_id')].append(row)
