...
class AccountInvoiceLine(models.Model):
    ...
    def create(self, vals):
        ...
        free_qty = vals.get('free_qty',0.0)
        if (qty == 0.0 and free_qty==0.0):
            raise Warning(_("Qty and Free qty both can't be zero, for %s") % vals.get('name',''))
        return super(AccountInvoiceLine, self).create(vals)
    
    @api.multi
    def write(self, vals):
        if vals.has_key('quantity') and vals.has_key('free_qty'):
            if vals.get('quantity',0.0) == 0.0 and vals.get('free_qty',0.0):
                raise Warning(_("Qty and Free qty both can't be zero in invoice lines!"))
