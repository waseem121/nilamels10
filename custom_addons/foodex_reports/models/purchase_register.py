...
class ReportPurchaseRegister(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        if data.get('picking_type_id',False):
            if data.get('vendor_ids'):
                self.env.cr.execute("SELECT id FROM purchase_order WHERE date_order::date>=%s and date_order::date<=%s and partner_id IN %s and picking_type_id=%s ORDER BY date_order", (data.get('date_from'),data.get('date_to'),(tuple(data.get('vendor_ids'))), data.get('picking_type_id')))
                query_res = self.env.cr.dictfetchall()
            else:
                self.env.cr.execute(
                    "SELECT id FROM purchase_order WHERE date_order::date>=%s and date_order::date<=%s and picking_type_id=%s ORDER BY date_order",
                    (data.get('date_from'), data.get('date_to'), data.get('picking_type_id')))
                query_res = self.env.cr.dictfetchall()
        else:
            if data.get('vendor_ids'):
