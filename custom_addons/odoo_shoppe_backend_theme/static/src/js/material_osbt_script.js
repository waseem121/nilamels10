...
var ThemeSwicher =  Widget.extend({
        ...
        switch_theme: function(theme){
            ...
            if (theme){
                ...
                new Model("res.users").call("color_switcher_write", [session.uid, theme]);
            }
        }
    });
...
var SystrayThemeSwitcher = Widget.extend({
    ...
    init:function(){
        ...
        new Model("res.users").call("read", [session.uid, ['theme', 'hide_theme_switcher', 'theme_lables_color', 'company_id']]).then(function(res){
            ...
            new Model("res.company").call("read", [res[0].company_id[0], ['theme_lables_color']]).then(function(res_company){
...
UserMenu.include({
    ...
    do_update: function () {
        ...
        new Model("res.company").call("read", [session.company_id, ['flag', 'flag_image']]).then(function(res){
            if(res[0].flag == "company_flag"){
                ...
                new Model("res.users").call("get_country_flag", [session.uid]).then(function(res){
                    if(res){
        ...
        new Model("res.users").call('get_user_time_format', [session.uid]).then(function(res){
...
var LeftUserMenu =  UserMenu.extend({
    ...
    on_menu_about: function () {
        var self = this;
        var help = ""
        new Model('res.company').call('about_company_data', [{}]).then(function (result) {
            help = result;
        });
        setTimeout(function(){
                new Dialog(self, {
