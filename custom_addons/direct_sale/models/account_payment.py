...
#            if self.partner_type == 'supplier':
    ...
    def default_get(self, fields):
        ...
        if len(journals):
            rec.update({'journal_id':journals[0].id})
    ...
    # inherited to add salesman and collector
    def _get_move_vals(self, journal=None):
        ...
        return {
            ...
            'receipt_no': self.receipt_no or name
        }
    ...
    def post(self):
        ...
        """
        for rec in self:
            print"rec.payment_type: ",rec.payment_type
            ...
            if rec.payment_type in ('inbound', 'outbound'):
                rec_account = rec.partner_id.property_account_receivable_id or False
                if not rec_account:
            ...
            # this should be called from Payment screen only and not from Register payment
            print"self contexttt: ",self._context
            print"payment type: ",rec.payment_type
            if rec.payment_type == 'inbound' and not self._context.get('active_model',False) :
#            if rec.payment_type == 'inbound':
...
#         
    ...
    # inherited to unreconcile the related JE
    @api.multi
    def cancel(self):
        for rec in self:
...
#                if rec.invoice_ids:
            ...
            if rec.invoice_ids:
                for inv in rec.invoice_ids:
                    for move_id in inv.move_id:
                        move_id.line_ids.remove_move_reconcile()
                        self.env.cr.execute("delete from account_invoice_payment_rel where payment_id=%s and invoice_id=%s",(rec.id,inv.id))
    ...
    def _onchange_partner_type(self):
        ...
        if self.partner_type:
            domain = {'partner_id': [(self.partner_type, '=', True), ('parent_id','=',False)]}
