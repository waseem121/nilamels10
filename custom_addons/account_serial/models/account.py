...
class AccountAccount(models.Model):
    ...
    def action_generate_serial_no(self):
        ...
        s1 = 0
        next_max_parent_serial_ids = self.search([
                                ('parent_id','!=',parent.id),
                                ('id','!=',self.id),
                                ('serial_no','>',parent.serial_no),
                                ], 
                                order='serial_no asc', limit=1)
        print"next_max_parent_serial_ids: ",next_max_parent_serial_ids
        next_max_parent_serial = 0
        if next_max_parent_serial_ids:
            next_max_parent_serial = next_max_parent_serial_ids[0].serial_no
        ...
        if next_max_parent_serial:
            domain.append(('serial_no','<',next_max_parent_serial))
        max_serial_account_ids = self.search(domain, order='serial_no desc', limit=1)
...
#            s1 = parent.serial_no + 1
        ...
        if not child_accounts:
            ...
            domain = [
                    ...
                    ('serial_no','!=',False),
                    ('serial_no','>',serial)]
            all_max_serial_account_ids = self.search(domain,
                                    order='serial_no ASC', limit=1)
