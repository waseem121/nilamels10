...
#        if True:
            if self.number:
                ...
                _logger.info('Execution time for pick validation (%s)', (time.time() - start_time,))
            ...
            if self.is_direct_invoice or self.refund_without_invoice or self.invoice_type=='transfer_invoice':
                if self.refund_invoice_id:
                    ...
                    _logger.info('Execution time for action_create_sales_order (%s)', (time.time() - start_time,))

                    start_time = time.time()
                    self.action_stock_transfer()
                    _logger.info('Execution time for action_stock_transfer (%s)', (time.time() - start_time,))
            ...
            _logger.info('Execution time for action_move_create (%s)', (time.time() - start_time,))

            start_time = time.time()
            self.check_date()
            _logger.info('Execution time for check_date (%s)', (time.time() - start_time,))

            start_time = time.time()
            to_open_invoices.invoice_validate()
            _logger.info('Execution time for invoice_validate (%s)', (time.time() - start_time,))
