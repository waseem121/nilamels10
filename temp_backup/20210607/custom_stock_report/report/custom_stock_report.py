...
#            category_ids = [prod_cat_id.id for prod_cat_id in obj.category_ids]
            ...
            category_ids = self.env['product.category'].search([('id', 'child_of', obj.category_ids.ids)])
            new_category_ids = category_ids
            category_ids = [c.id for c in category_ids]
            print"category_ids: ",category_ids
            print"category_ids len: ",len(category_ids)
...
#            print"query res: ",res
    ...
    #            print"res: ",res
                final_dict = {}
                print"new_category_ids: ",new_category_ids
                if obj.group_by == 'category':
                    team_dict = defaultdict(list)
                    categ_ids_to_use = []
                    for category in new_category_ids:
                        ...
                        categ_ids_to_use = new_category_ids.ids # TODO
...
#                            d['carton'] = carton
                        ...
                        res.append(d)
                    fdict[key][prod] += res                    
#            print"fdict kesysss: ",fdict.keys()
#            print"fdict kesysss len: ",len(fdict.keys())
            return fdict
                    
            # not needed
            if not obj.stock_lot in ['with_lot']:
                fdict = {}
                for key,value in final_dict.items():
                    if key not in fdict:
                        fdict.update({key: {}})
