...
class sale_analysis_report(models.Model):
    ...
    commission = fields.Float('Commission', readonly=True, digits=dp.get_precision('Discount'))

#SUM(CASE WHEN s.type = 'out_invoice' THEN (l.price_subtotal_signed - ((l.quantity+l.free_qty) * l.cost_price)) ELSE (l.price_subtotal_signed - ((l.quantity+l.free_qty) * l.cost_price)) * -1 END) as profit,
#SUM(CASE WHEN s.type = 'out_invoice' THEN l.discount_amount ELSE l.discount_amount * -1 END) as discount,

#    update account_invoice_line set free_qty=0 where free_qty is null;
#    update account_invoice_line set total_discount_amount=0 where total_discount_amount is null;
    def _select(self):
        select_str = """
            WITH currency_rate as (%s)
             SELECT min(l.id) as id,
                    ...
                    SUM(CASE WHEN s.type = 'out_invoice' THEN (l.price_subtotal_signed-l.total_discount_amount) ELSE (l.price_subtotal_signed-COALESCE(l.total_discount_amount,0)) * 1 END) as price_subtotal,

                    SUM(CASE WHEN s.type = 'out_invoice' THEN COALESCE(l.total_discount_amount,0) ELSE COALESCE(l.total_discount_amount,0) * -1 END) as discount,
                    ...
                    SUM(CASE WHEN s.type = 'out_invoice' THEN ((l.price_subtotal_signed-COALESCE(l.total_discount_amount,0)-COALESCE(l.commission_share,0)) - ((l.quantity+l.free_qty) * l.cost_price)) ELSE (((l.price_subtotal_signed*-1)-COALESCE(l.total_discount_amount)-COALESCE(l.commission_share)) - ((l.quantity+l.free_qty) * l.cost_price)) * -1 END) as profit,
...
#                            left join product_template t on (p.product_tmpl_id=t.id)
#                    left join product_pricelist pp on (s.pricelist_id = pp.id)
#                    left join res_currency_rate cr on (cr.currency_id = s.currency_id AND
#                        cr.company_id = s.company_id AND
#                        cr.date_start <= COALESCE(s.date_invoice, NOW()) AND
#                        cr.date_end IS NULL OR cr.date_end > COALESCE(s.date_invoice, NOW()))
        #"""
