...
#        return {}
        datas = {
            ...
            'category': self.category_id and str(self.category_id.name)+ '  and its sub-Categories' or 'All',
...
class ReportSalesAnalysisGroup(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        wizard_categ_ids = []
        wizard_category_id = data.get('category_id',False)
        wchild_categ_ids = self.env['product.category'].search([('id', 'child_of', wizard_category_id)])
        if len(wchild_categ_ids):
            wizard_categ_ids = wchild_categ_ids.ids
        wizard_categ_ids.append(wizard_category_id)
        wizard_categ_ids = list(set(wizard_categ_ids))
...
#            print"group_account name: ",group_account.name
...
#                if wizard_category_id and category.id not in wizard_categ_ids:
#                    print"continueee"
#                    continue
...
#                print"invoiceidsss: ",invoice_ids
                ...
                for invoice in invoice_ids:
                    ...
                    for l in invoice.invoice_line_ids:
                        if wizard_category_id:
                            if l.product_id.categ_id.id not in wizard_categ_ids:
                                continue
                        
                        if l.product_id.categ_id.id in child_category_ids:
                           net_sales =  (l.price_subtotal/exchange_rate) - (l.discount_share/exchange_rate) - (l.commission_share/exchange_rate)
#                           net_cost =  (l.cost_price/exchange_rate) * (l.quantity + l.free_qty)
                           net_cost =  (l.cost_price) * (l.quantity + l.free_qty)
                           net_profit = net_sales - net_cost
                           
                           this_sales += net_sales
                           this_cost += net_cost
                           this_profit += net_profit
                           
                    if invoice.type == 'out_refund':
                        this_sales = this_sales * -1
                        this_cost = this_cost * -1
                        this_profit = this_profit * -1
...
#                print"categ_values: ",categ_values
