...
class InsFinancialReportXlsx(ReportXlsx):
...
#            Child starts
                if a['account_type'] == 'view':
                    for l in a['child_lines']:
                        if l['level'] == 2:
                            self.row_pos += 1
                        self.row_pos += 1
                        if l.get('account', False):
                            tmp_style_str = self.line_header_string
                            tmp_style_num = self.line_header
                        else:
                            tmp_style_str = self.line_header_string_bold
                            tmp_style_num = self.line_header_bold
                        self.sheet.write_string(self.row_pos, 0, '   ' * len(l.get('list_len', [])) + l.get('name'),
                                                tmp_style_str)
                        self.sheet.write_number(self.row_pos, 1, float(l.get('debit')), tmp_style_num)
                        self.sheet.write_number(self.row_pos, 2, float(l.get('credit')), tmp_style_num)
                        self.sheet.write_number(self.row_pos, 3, float(l.get('balance')), tmp_style_num)
                        
#                grand child starts
#                        if l['account_type'] == 'view':
                        for gl in l['grand_child_lines']:
                            if gl['level'] == 2:
                                self.row_pos += 1
                            self.row_pos += 1
                            if gl.get('account', False):
                                tmp_style_str = self.line_header_string
                                tmp_style_num = self.line_header
                            else:
                                tmp_style_str = self.line_header_string_bold
                                tmp_style_num = self.line_header_bold
                            self.sheet.write_string(self.row_pos, 0, '   ' * len(gl.get('list_len', [])) + gl.get('name'),
                                                    tmp_style_str)
                            self.sheet.write_number(self.row_pos, 1, float(gl.get('debit')), tmp_style_num)
                            self.sheet.write_number(self.row_pos, 2, float(gl.get('credit')), tmp_style_num)
                            self.sheet.write_number(self.row_pos, 3, float(gl.get('balance')), tmp_style_num)
...
InsFinancialReportXlsx('report.dynamic_xlsx.ins_financial_report_xlsx','ins.financial.report')