...
#            return res
        ...
        for each_res in res:
            ...
            if self.type == 'in_invoice':
                ...
                each_res['exchange_rate'] = self.exchange_rate or 0.0
...
#                res['name'] = '/'
        ...
        if line.get('exchange_rate',0):
            res['exchange_rate'] = line.get('exchange_rate',0)
        return res
...
#        print"zero_stock_product_ids: ",zero_stock_product_ids
...
#    total_discount_share = fields.Monetary(string='Total Discount Share', compute='_compute_discount', store=True)
    ...
    exchange_rate = fields.Float(string='Exchange Rate', digits=dp.get_precision('Discount'), default=0.0, copy=False)
...
#    cost_price = fields.Float(string="Cost Price", digits=dp.get_precision('Discount'))
    cost_price = fields.Float(string="Cost Price", digits=(12, 10))
...
class AccountMoveLine(models.Model):
    ...
    debit = fields.Float(default=0.0, digits=(12, 10))
    credit = fields.Float(default=0.0,digits=(12, 10))
...
#        return {}
    ...
    @api.onchange('debit','credit','amount_currency')
    def _onchange_debit_credit_amount_currency(self):
        if self.amount_currency and (self.debit or self.credit):
            if self.debit>0:
                self.exchange_rate = abs(self.amount_currency) / self.debit
            elif self.credit>0:
                self.exchange_rate = abs(self.amount_currency) / self.credit
        return {}
    ...
    def create(self, vals):
        ...
        print"valss: ",vals
        print"vals.get('amount_currency',0): ",vals.get('amount_currency',0)
        
        if currency and abs(vals.get('amount_currency',0))>0 and not vals.get('exchange_rate'):
            message = 'Please enter exchange rate'
            if vals.get('account_id'):
                name = self.env['account.account'].browse(vals['account_id']).name
                message = message + ' for line '+str(name)
