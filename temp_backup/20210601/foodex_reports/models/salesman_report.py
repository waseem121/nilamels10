...
class ReportsalesmanSales(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        self.clear_caches()
        domain = [('user_id','=',data.get('salesman_id')),
                    ('date_invoice', '>=', data.get('date_from')),
                    ('date_invoice', '<=', data.get('date_to')), 
#                    ('type', '=', 'out_invoice'),
        ...
        if invoice_ids:
            total = sum(invoice_ids.filtered(lambda x: x.type == 'out_invoice').mapped('amount_total'))
            refund_total = sum(invoice_ids.filtered(lambda x: x.type == 'out_refund').mapped('amount_total'))
            total -= refund_total
