...
#        domain.append(('invoice_id', 'in', (431,435)))
        start = datetime.now()
        invoice_line_ids = self.env['account.invoice.line'].search(domain)
        end = datetime.now()
...
#        invoice_ids = self.env['account.invoice'].search([('id','=',435)])
            ...
            if len(final_res):
                ...
                for this_res in final_res:
                    ...
                    if invoice_id and (invoice_id == l.invoice_id.id):
                        skip=True
                        this_total = l.price_subtotal
                        this_cost = l.cost_price* (l.quantity + l.free_qty)
                        
                        discount_share = l.discount_share
                        commission_share = l.commission_share
                        
#                        this_discount = this_res['discount'] + (discount_share or 0.0)
                        this_profit = this_total - this_cost
                        this_net_sales = this_total - discount_share -commission_share
#                        this_net_sales = this_res['net_sales'] + (this_total - discount_share -commission_share)
#                        this_extras = this_res['extras'] + commission_share
                        ...
                        this_total_d = this_total
                        this_cost_d = this_cost
                        if this_total_d == 0.0: this_total_d = 1.0
                        if this_cost_d == 0.0: this_cost_d = 1.0

                        profit_percent_sales = (this_profit / this_total_d) * 100.0
                        profit_percent_cost = (this_profit / this_cost_d) * 100.0

                        if invoice.type == 'out_refund':
                            this_profit_percent_sales = this_res['profit_percent_sales'] - profit_percent_sales
                            this_profit_percent_cost = this_res['profit_percent_cost'] - profit_percent_cost
                        else:
                            this_profit_percent_sales = this_res['profit_percent_sales'] + profit_percent_sales
                            this_profit_percent_cost = this_res['profit_percent_cost'] + profit_percent_cost
                        
                        this_res['profit_percent_sales'] = this_profit_percent_sales
                        this_res['profit_percent_cost'] = this_profit_percent_cost
                        ...
                        if invoice.type == 'out_refund':
                            ...
                            total_profit -= this_total - (l.cost_price* (l.quantity + l.free_qty))
                            total_extras -= commission_share
                            total_net_sales -= (this_total - discount_share - commission_share)
                            total_cost -= l.cost_price* (l.quantity + l.free_qty)
                        else:
                            total_sales += this_total
                            total_discount += discount_share or 0.0
                            total_profit += this_total - (l.cost_price* (l.quantity + l.free_qty))
                        ...
                        d['profit_percent_sales'] = profit_percent_sales
                        d['profit_percent_cost'] = profit_percent_cost
...
#                    print"time 1111",(str(end - start))
                ...
                if this_total == 0.0: this_total=1.0
                if this_cost == 0.0: this_cost=1.0
                profit_percent_sales = (this_profit / this_total) * 100.0
                profit_percent_cost = (this_profit / this_cost) * 100.0
