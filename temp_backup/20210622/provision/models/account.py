...
#                job.write({'state': 'invoiced'})
    ...
    #                        {'location': self.location_id.id, 'compute_child': False}).virtual_available_inv
        ...
        ## adding debit lines to the draft payment reconcilation lines if exists
        self._add_lines_to_draft_payment()
        return to_open_invoices.invoice_validate()    
    
    def _add_lines_to_draft_payment(self):
        if self.type == 'out_invoice' and self.partner_id:
            payment_ids = self.env['account.payment'].search([
                                ('partner_id','=',self.partner_id.id),
                                ('payment_type','=','inbound'),
                                ('state','=','draft')])
                                
            if payment_ids and self.move_id:
                if self.partner_id.property_account_receivable_id:
                    ...
                    for debit_line in debit_move_lines:
                        ...
                        self.env['manual.reconcilation.lines'].create(vals)
