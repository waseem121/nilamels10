...
class ManualReconcilationLines(models.Model):
    ...
    name = fields.Char(string='Move Name',related='debit_line_id.move_id.name')
...
#        self.manual_reconcilation_lines = [(2, move.id) for move in self.move_raw_ids.filtered(lambda m: m.bom_line_id)]
    ...
    def manual_reconcile(self):
        ...
        if float(round(amount_to_reconcile,6)) > float(round(total_amount,6)):
...
#            total_diff = sum(self.manual_reconcilation_lines.mapped('balance')) - sum(self.manual_reconcilation_lines.mapped('amount_to_reconcile'))
            total_diff = sum((self.manual_reconcilation_lines.filtered(lambda x: x.amount_to_reconcile>0)).mapped('balance')) - sum(self.manual_reconcilation_lines.mapped('amount_to_reconcile'))
