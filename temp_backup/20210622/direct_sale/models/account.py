...
#    customer_div_id = fields.Many2one('customer.division', string="Customer Division")
    ...
    def copy_custom(self):
        ...
        for l in self.invoice_line_ids_two:
            ...
            v['free_qty'] = l.free_qty
            v['uom_id'] = l.uom_id.id
#            v['cost_price'] = l.cost_price
            v['price_unit'] = l.price_unit
            v['discount'] = l.discount
            v['discount_amount'] = l.discount_amount
            v['price_subtotal'] = l.price_subtotal
            v['account_id'] = l.account_id and l.account_id.id
            v['invoice_date'] = l.invoice_date
            invoice_line_ids.append((0, 0, v))
            i += 1
            
        ctx['default_invoice_line_ids_two'] = invoice_line_ids
#        ctx['default_invoice_line_ids'] = invoice_line_ids
        
        ctx['default_amount_untaxed'] = self.amount_untaxed
        ctx['default_amount_untaxed_wo_global_disc'] = self.amount_untaxed_wo_global_disc
        ctx['default_item_discount'] = self.item_discount
        ctx['default_total_discount'] = self.total_discount
#        ctx['default_total_cost'] = self.total_cost
        ctx['default_amount_total'] = self.amount_total
        ctx['default_discount_type'] = self.discount_type
        ctx['default_discount_glob'] = self.discount_glob
        ctx['default_commission'] = self.commission
        ctx['default_commission_account_id'] = self.commission_account_id and self.commission_account_id.id or False
        ctx['default_type'] = self.type
        ctx['default_location_id'] = self.location_id and self.location_id.id or False
        ctx['default_user_id'] = self.user_id and self.user_id.id or False
        ctx['default_is_direct_invoice'] = self.is_direct_invoice or False
        ctx['default_invoice_type'] = self.invoice_type
...
#            return res
        ...
        for each_res in res:
            ...
            if self.type == 'in_invoice':
                name = self.reference or self.origin
#                name = self.origin or '/'
...
#                res['name'] = '/'
            ...
            if line.get('amount_currency'):
                res['exchange_rate'] = line.get('exchange_rate',0.0) or inv.exchange_rate
        return res
...
#                line['price'] = self.currency_id.round(line['price'])
                line['price'] = float(round(line['price'],6))
...
#                    final_qty = lines.quantity if split_qty != lines.quantity else each_split_line.split_qty
    ...
    def do_return_picking(self):
        ...
        stock_return_default_value = stock_return_picking.with_context(
            {'active_id': picking_id.id,
            'refund_invoice_id':self.id}).default_get(lst)
        stock_return_picking_id = stock_return_picking.with_context(
            {'active_id': picking_id.id,
            'refund_picking_id':self.id}).create(
            stock_return_default_value)
#        return_picking_id = stock_return_picking_id.with_context({'active_id': picking_id.id})._create_returns()
        return_picking_id = stock_return_picking_id.with_context(
            {'active_id': picking_id.id,
            'refund_invoice_id':self.id})._create_returns()
...
#                name = 'Credit Invoice '+str(name)
            ...
            if inv.type == 'in_invoice':
                name = inv.reference or inv.origin
#                name = inv.origin or '/'
            ...
            if inv.payment_term_id:
                ...
                for i, t in enumerate(totlines):
                    ...
                    iml.append({
                        ...
                        'exchange_rate': diff_currency and inv.exchange_rate,
...
#                ref = inv.reference or inv.origin
                ref = inv.origin    # 2021 new syn changes
            move_vals = {
#                'ref': inv.reference,
                'ref': ref,
                        ...
                        if picking.picking_type_id.code=='outgoing':
                            ...
                            for quant in move.quant_ids:
                                    ...
                                    if len(res):
                                        ...
                                        if not quant_updated:
                                            ...
                                            if product_cost != quant_cost:
                                                ...
                                                new_qty = quant.qty
                                                print"new_qty: ",new_qty
                                                avg_price = ((old_qty * product_cost) + (new_qty * quant_cost)) / (old_qty + new_qty)
                                ...
                                if move.product_qty < quant.qty:
                                    ...
                                    quant.write({'qty':float(quant.qty-move.product_qty)})
                                    quant_created = stock_quant_sudo.create({'product_id':move.product_id.id,
                                                    'location_id':move.location_id.id,
                                ...
                                if move.product_qty > quant.qty:
                                    diff = move.product_qty - quant.qty
                                    quant.write({'location_id':move.location_id.id,
                                        'qty':move.product_qty})
                                    quant_created = stock_quant_sudo.create({'product_id':move.product_id.id,
...
#                                
#                                for quant in move.quant_ids:
#                                    print"quant locaiton: ",quant.location_id
#                                    print"Move locaiton: ",move.location_id
#                                    ee
#                                    if quant_updated:
#                                        continue
...
#                                        # find customer quant ids
#                                        self._cr.execute("""SELECT id FROM stock_quant WHERE product_id = %s and location_id=%s and id !=%s""", (quant.product_id.id, move.location_id.id, quant.id))
#                                        res = self._cr.fetchall()
#                                        print"query res1 customer_quant_ids: ",res
#                                        if len(res):
#                                            customer_quant_ids = [x[0] for x in res]
#                                            print"customer_quant_ids: ",customer_quant_ids
#                                            
#                                            # find the cusomer location move to make relation with
#                                            self._cr.execute("""SELECT move_id FROM stock_quant_move_rel WHERE quant_id = %s""", (customer_quant_ids[0],))
#                                            res = self._cr.fetchall()
#                                            print"query res1 customer moves: ",res
#                                            if len(res):
#                                                customer_move_ids = [x[0] for x in res]
#                                                print"customer_move_ids: ",customer_move_ids
#                                                if len(customer_move_ids):
#                                                    relation_created=False
#                                                    for m in Move.browse(customer_move_ids):
##                                                        if relation_created:
##                                                            continue
#                                                        print"m location: ",m.location_id
#                                                        print"m location dest_id: ",m.location_dest_id
#                                                        # if both are customer locations
#                                                        if m.location_dest_id.id == move.location_id.id:
#                                                            self._cr.execute("""SELECT move_id FROM stock_quant_move_rel WHERE quant_id = %s and move_id=%s""", (quant.id,m.id))
#                                                            existing_rel = self._cr.fetchall()
#                                                            print"existing_rel1: ",existing_rel
...
#                                        quant.write({'location_id':move.location_id.id})
#                                        quant_updated=True
#                                        print"quant  updated with location"
#                                    ## second scnario
#                                    if quant.qty < move.product_qty:
#                                        print("in second scenario")
#                                        quant_created = stock_quant_sudo.create({'product_id':move.product_id.id,
#                                                        'location_id':move.location_dest_id.id,
#                                                        'qty':move.product_qty*-1,
#                                                        'in_date':date.today().strftime(DT)})
#                                        print"second scenario quant_created yesssss: ",quant_created
#                                        
#                                        
#                                        
#                                        self._cr.execute("""SELECT id FROM stock_quant WHERE product_id = %s and location_id=%s and id !=%s""", (quant.product_id.id, move.location_id.id, quant.id))
...
#                                                        print"m location dest_id: ",m.location_dest_id
#                                                        if m.location_dest_id.id == move.location_id.id:
#                                                            self._cr.execute("""SELECT move_id FROM stock_quant_move_rel WHERE quant_id = %s and move_id=%s""", (quant_created.id,m.id))
#                                                            existing_rel = self._cr.fetchall()
#                                                            print"existing_rel1: ",existing_rel
#                                                            if len(existing_rel):
#                                                                print"continueee111"
...
#                                                                VALUES(%s,%s)
                                                                #""", (m.id,quant_created.id))
#                                                            print"relation created move_id,quant_id:1 ",m.id,quant_created.id
##                                                            relation_created=True
#                                                    print"after for loop"
#                                        print"before quant udpate"
#                                        ## updating qty in customer locaiton quant
#                                        quant.write({'location_id':move.location_id.id,
#                                            'qty':move.product_qty})
#                                        quant_updated=True
                                        
#                                        # qty has tobe substracted from store other quant
                                        ## its working but not recomended
#                                        self._cr.execute("""SELECT id FROM stock_quant WHERE product_id = %s and location_id=%s and id != %s""", (quant.product_id.id, move.location_dest_id.id, quant.id))
#                                        res = self._cr.fetchall()
#                                        print"Sub query res1 store_quant_ids: ",res
#                                        if len(res):
#                                            store_quant_ids = [x[0] for x in res]
#                                            print"store_quant_ids: ",store_quant_ids
#                                            if len(store_quant_ids):
#                                                store_quant = stock_quant_sudo.browse(store_quant_ids)
#                                                qty_substracted=False
#                                                for q in store_quant:
#                                                    if qty_substracted:
#                                                        continue
#                                                    if q.qty >=move.product_qty:
#                                                        q.write({'qty':q.qty-move.product_qty})
#                                                        qty_substracted=True
#                                                        print"q substracted: ",q
#                                                print"qty_substracted: ",qty_substracted
#                                                if not qty_substracted:
#                                                    move_qty = move.product_qty
#                                                    for q in store_quant:
...
#                    stop     
    ...
    def purchase_order_change(self):
        ...
        print"self.purchase Currency : ",self.purchase_id.currency_id.name
        ...
        print"currency after: ",self.currency_id.name
        return {}
...
#            'price_unit': line.order_id.currency_id.with_context(date=self.date_invoice).compute(line.price_unit, self.currency_id, round=False),
    ...
    def unlink(self):
        for invoice in self:
            if invoice.amount_total==0 and invoice.type=='in_invoice':
                move = invoice.move_id or False
                invoice.state='cancel'
                invoice.move_id=False
                if move and move.amount==0:
                    move.unlink()
...
#                        taxes['total_excluded'] if taxes else round((
...
#            vals['name']='/'
                ...
                if invoice:
                    ...
                    if journal.sequence_id:
                        ...
                        if invoice and invoice.type in ['out_refund', 'in_refund'] and journal.refund_sequence:
                            ...
                            sequence = journal.refund_sequence_id
                        print "\n direct sale create called for sequence :::::::::::::::::::"
    ...
    def copy_custom(self):
        ctx = {}
        ctx['default_journal_id'] = self.journal_id and self.journal_id.id or False
        ctx['default_salesman_id'] = self.salesman_id and self.salesman_id.id or False
        ctx['default_collector_id'] = self.collector_id and self.collector_id.id or False
        ctx['default_division_id'] = self.division_id and self.division_id.id or False
        ctx['default_department_id'] = self.department_id and self.department_id.id or False
        ctx['default_currency_id'] = self.currency_id and self.currency_id.id or False
        
        ctx['default_amount'] = self.amount
        ctx['default_state'] = 'draft'
        ctx['default_narration'] = self.narration
        ctx['default_ref_required'] = self.ref_required
...
#            v['ref'] = False
            ...
            v['quantity'] = l.quantity
            v['reconciled'] = False
            v['bank_reconcile'] = False
            v['x_particulars'] = l.x_particulars
            v['exchange_rate'] = l.exchange_rate
            
            v['partner_id'] = l.partner_id and l.partner_id.id or False
            v['journal_id'] = l.journal_id and l.journal_id.id or False
            v['account_id'] = l.account_id and l.account_id.id or False
            v['product_id'] = l.product_id and l.product_id.id or False
            v['payment_id'] = l.payment_id and l.payment_id.id or False
            v['company_currency_id'] = l.company_currency_id and l.company_currency_id.id or False
            v['product_uom_id'] = l.product_uom_id and l.product_uom_id.id or False
            v['customer_division_id'] = l.customer_division_id and l.customer_division_id.id or False
            v['customer_department_id'] = l.customer_department_id and l.customer_department_id.id or False

            line_ids.append((0, 0, v))
            
        ctx['default_line_ids'] = line_ids
        view = self.env.ref('direct_sale.view_move_form_data_entry_direct_sale_syn', False)
        return {
            'name': _('Journal Entries Data Entry'),
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
...
class AccountMoveLine(models.Model):
    ...
    bank_reconcile = fields.Boolean(string="Bank Reconcile", default=False)
...
class AccountAccount(models.Model):
    ...
    bank_reconcile = fields.Boolean(string='Bank Reconcile', default=False, 
        help="Enables Bank reconcilation process.")
...
#            action_context = {'show_mode_selector': False, 'mode': 'suppliers'}
    ...
    @api.multi
    def action_open_bank_reconcile(self):
        
        move_ids = self.env['account.move.line'].search([('account_id','=',self.id)])
        if len(move_ids):
            domain = [('id', 'in', tuple(move_ids.ids))]

        
        action = self.env.ref('direct_sale.act_bank_reconcilation').read()[0]
        move_ids = self.env['account.move.line'].search([('account_id','=',self.id)])
        print"move_ids: ",move_ids
        if len(move_ids):
            action['domain'] = [('id', 'in', tuple(move_ids.ids))]
        else:
            action['domain'] = [('id', 'in', [])]
#        print"action: ",action
        return action
