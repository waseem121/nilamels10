...
#                    0].sale_line_ids else self.date_invoice,
            ...
            moves = self.invoice_line_ids.filtered(
                ...
                picking)
            print"self.invoice_picking_id.pack_operation_product_ids11: before assign ",self.invoice_picking_id.pack_operation_product_ids
            picking.action_assign()
            print"self.invoice_picking_id.pack_operation_product_ids after assign: ",self.invoice_picking_id.pack_operation_product_ids
            picking.force_assign()
            print"self.invoice_picking_id.pack_operation_product_ids after Force assign: ",self.invoice_picking_id.pack_operation_product_ids
    ...
    def assign_lot_pack_op(self):
        ...
        product_group_by = {}
        product_ids = []
        for l in self.invoice_line_ids.filtered(lambda l: l.product_id):
            product_ids.append(l.product_id.id)
        print"product_idss: ",product_ids
        product_ids = list(set(product_ids))
        for product_id in product_ids:
            print"product_id: ",product_id
            for lines in self.invoice_line_ids.filtered(lambda l: l.product_id.id==product_id):
                print"linesssssssssssssssss: ",lines
                ...
                if lines.product_id.tracking != 'none':
                    split_lot_obj = split_lot_obj.search([('account_invoice_line_id', '=', lines.id)])
                    print"split_lot_obj: ",split_lot_obj
                    total_qty = 0
                    for each_split_line in split_lot_obj.split_lot_qty_line_ids:
                        print"each_split_line.split_qty: ",each_split_line.split_qty
    #                    total_qty += each_split_line.split_qty
                        total_qty += (lines.quantity + lines.free_qty)
                        if not lines.product_id.id in product_group_by:
                            product_group_by.update({lines.product_id.id: []})
                        product_group_by[lines.product_id.id].append({'lot_id': each_split_line.lot_id.id,
    #                                                                  'qty_todo': each_split_line.split_qty,
    #                                                                  'qty': each_split_line.split_qty
                                                                      'qty_todo': lines.quantity + lines.free_qty,
                                                                      'qty': lines.quantity + lines.free_qty
                                                                      })
        ...
        print"self.invoice_picking_id.pack_operation_product_ids: ",self.invoice_picking_id.pack_operation_product_ids
        for pack_opt in self.invoice_picking_id.pack_operation_product_ids:
            if pack_opt.product_id.tracking != 'none':
                pack_opt.pack_lot_ids = False
                for product, lotvals in product_group_by.items():
                    if pack_opt.product_id.id == product:
                        ...
                        for k, v in groupby(sorted(lotvals, key=key), key=key):
                            ...
                            for each in v:
                                qty += float(each['qty'])
                                qty_todo += float(each['qty_todo'])
                            print"directsale/account.py product: ",product
                            print"directsale/account.py qtyyyy: ",qty
                            lot_total_qty.append((0, 0, {'lot_id': k, 'qty': qty, 'qty_todo': qty_todo}))
                        pack_opt.write({'pack_lot_ids': lot_total_qty})
