...
#        print"account_report: ",account_report
...
#        print"resss: ",res
        ...
        for report in child_reports:
            ...
            if res[report.id].get('account'):
                sub_lines = []
                view_account_ids = []
#                print"res[report.id]['account'].items():====== ",res[report.id]['account'].items()
...
#                    print"account_id: ",account_id
                    ...
                    if account.parent_id:
                        if account.parent_id.type == 'view':
                            view_account_ids.append(account.parent_id.id)
                        if account.parent_id.parent_id:
                            if account.parent_id.parent_id.type == 'view':
                                view_account_ids.append(account.parent_id.parent_id.id)
                    vals = {
                        'account': account.id,
                        'parent_account_id': account.parent_id and account.parent_id.id or False,
...
#                    flag = True # custom
                ...
                #custom
                if len(view_account_ids):
                    sub_lines_new = []
                    view_account_ids = list(set(view_account_ids))
#                    print"view_account_ids: ",view_account_ids
                    view_accounts = self.env['account.account'].browse(list(set(view_account_ids)))
                    main_view_account = False
                    for a in view_accounts:
                        if not a.parent_id:
                            main_view_account = a
                    new_view_accounts = self.env['account.account'].search([('parent_id','=',main_view_account.id)])
                    
                    for view_account in new_view_accounts:
                        child_view_accounts = self.env['account.account'].search(
                            [('parent_id','=',view_account.id)])
                        child_lines = []
                        total_debit, total_credit, total_balance = 0.0, 0.0, 0.0
                        final_debit, final_credit, final_balance = 0.0, 0.0, 0.0
                        if len(child_view_accounts):
                            for child_view_account in child_view_accounts:
#                                print"child_view_account nameeeeeee :",child_view_account.name
                                debit, credit, balance = 0.0, 0.0, 0.0
                                grand_child_lines = []
                                for v in sub_lines:
                                    if v['parent_account_id'] == child_view_account.id:
                                        v['parent'] = child_view_account.id
                                        v['account_type']='account_type'
                                        v['type']='report'
                                        v['level']=4
                                        v['style_type']='sub'
                                        v['list_len'] = [a for a in range(0,report.display_detail == 'detail_with_hierarchy' and 8)]
                                        debit += v['debit']
                                        credit += v['credit']
                                        balance += v['balance']
#                                        v['balance'] = v['balance'] * int(report.sign)
#                                        balance += v['balance']1
                                        grand_child_lines.append(v)
...
#                                        total_balance += balance
                                ...
                                for account_id, value in res[report.id]['account'].items():
                                    if account_id == child_view_account.id:
                                        debit = value['debit']
                                        credit = value['credit']
                                        balance = value['balance']
#                                        balance = value['balance'] * int(report.sign) or 0.0
                                ...
                                vals = {
                                    'account': child_view_account.id,
                                    'code': int(child_view_account.code),
                                    'name': child_view_account.code + ' ' + child_view_account.name,
                                    'debit':debit,
                                    'credit':credit,
                                    'balance':balance,
#                                    'balance': value['balance'] * int(report.sign) or 0.0,
                                    'type': 'report',
                                    'parent': view_account.id,
                                    'self_id': 50,
            #                        'style_type': 'sub',
                                    'style_type': 'main',
                                    'precision': currency_id.decimal_places,
                                    ...
                                    'position': currency_id.position,
#                                    'list_len':[a for a in range(0,report.display_detail == 'detail_with_hierarchy' and 4)],
                                    'list_len':[a for a in range(0,report.display_detail == 'detail_with_hierarchy' and 6)],
                                    'level': 4,
                                    'company_currency_id': company_id.currency_id.id,
                                    'account_type': 'account_type',
                                    'grand_child_lines': grand_child_lines,
                                }
#                                if child_view_account.name == 'Total Cost Of sales':
#                                    print"valsss: ",vals
                                child_lines.append(vals)
                                final_debit += debit
                                final_credit += credit
                                final_balance += balance
#                        print"sub_lines_new: ",sub_lines_new
#                        if not len(child_view_accounts):
                        found = False
                        for account_id, value in res[report.id]['account'].items():
                            if account_id == view_account.id:
                        ...
                        vals = {
                            'account': view_account.id,
                            'code': int(view_account.code),
                            'name': view_account.code + ' ' + view_account.name,
                            'debit': total_debit,
                            'credit': total_credit,
...
#                            'account_type': account.type,
                        ...
                        sub_lines_new.append(vals)
                    
#                    child_ids = self.env['account.account'].search([('parent_id','=',view_account.id)])
                #custom
                
#                lines += sorted(sub_lines, key=lambda sub_line: sub_line['name'])
                lines += sorted(sub_lines_new, key=lambda sub_line: sub_line['name'])
